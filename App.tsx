...
export default function App() {
  ...
  // State for history payment toggle
  ...
  const togglePaymentStatus = async (rentalId: string, currentStatus: boolean | undefined) => {
    const newStatus = !currentStatus;

    try {
      if (useSupabase) {
        const { error } = await supabase.from('rentals').update({ is_paid: newStatus }).eq('id', rentalId);
        if (error) throw error;
      }
      setRentals(prev => prev.map(r => r.id === rentalId ? { ...r, isPaid: newStatus } : r));
    } catch (e) {
      alert("Failed to update payment status: " + getErrorMessage(e));
    }
  };
        ...
        {consoles.map(console => {
          ...
          return (
            <Card
              ...
              className={`cursor-pointer transition-all hover:border-violet-500/50 group relative overflow-hidden ${console.status === ConsoleStatus.IN_USE ? 'border-rose-500/30' : ''
                }`}
            >
              <div className={`absolute -right-4 -top-4 w-24 h-24 rounded-full blur-2xl opacity-20 ${console.status === ConsoleStatus.AVAILABLE ? 'bg-emerald-500' :
          ...
          <Card className="bg-slate-800/50">
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center gap-2 mt-3 pt-2 border-t border-slate-700">
                <input
                  type="checkbox"
                  id={`paid-${activeRental.id}`}
                  checked={activeRental.isPaid || false}
                  onChange={() => togglePaymentStatus(activeRental.id, activeRental.isPaid)}
                  className="w-4 h-4 rounded border-slate-600 bg-slate-800 text-emerald-500 focus:ring-emerald-500 cursor-pointer"
                />
                <label htmlFor={`paid-${activeRental.id}`} className={`text-sm font-bold cursor-pointer select-none ${activeRental.isPaid ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {activeRental.isPaid ? 'PAID' : 'Mark as Paid'}
                </label>
              </div>
            </div>
          </Card>
              ...
              {members.map(m => {
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-lg border text-left transition-all relative ${tempMemberId === m.id
        ...
        <div className="flex bg-slate-800 p-1 rounded-lg">
          {(['daily', 'weekly', 'monthly'] as const).map(f => (
            <button
              ...
              className={`flex-1 py-1.5 text-xs font-bold uppercase tracking-wider rounded-md transition-all ${historyFilter === f ? 'bg-violet-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'
                }`}
            >
              {f}
            </button>
                ...
                return (
                  <Card key={`h_${idx}`} className="flex justify-between items-center border-l-4 border-l-violet-500">
                    ...
                    <div className="text-right">
                      ...
                      <button
                        onClick={() => {
                          setSelectedHistoryRental(item);
                          setShowHistoryPaymentModal(true);
                        }}
                        className={`text-[10px] font-bold px-1.5 py-0.5 rounded cursor-pointer hover:opacity-80 active:scale-95 transition-all ${item.isPaid ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}
                      >
              ...
              <div className="relative z-10">
                ...
                <div className="flex justify-between items-start mb-5">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg shadow-inner ring-1 ring-white/10 ${activePkg
                  ...
                  {activePkg ? (
                    <div className="text-right">
                      <span className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-lg border shadow-sm ${activePkg.type === 'Juragan'
                  ...
                  <div className="space-y-3 pt-3 border-t border-slate-800/60">
                    ...
                    <div>
                      ...
                      <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${(activePkg.remainingMinutes / activePkg.initialMinutes) < 0.2 ? 'bg-rose-500' : 'bg-violet-500'
                            }`}
                      ...
                      <div>
                        ...
                        <div className="flex gap-1.5">
                          {Array.from({ length: activePkg.initialDrinks }).map((_, i) => (
                            <div
                              key={i}
                              className={`h-1.5 flex-1 rounded-full transition-colors ${i < activePkg.remainingDrinks ? 'bg-amber-500' : 'bg-slate-800'
                                }`}
                            />
                          ))}
                        </div>
      ...
      <div className="flex overflow-x-auto gap-2 pb-2 scrollbar-hide">
        {(['pricing', 'consoles', 'items', 'staff'] as const).map(tab => (
          <button
            ...
            className={`px-4 py-2 rounded-lg font-bold text-sm capitalize whitespace-nowrap transition-colors ${settingsTab === tab
  ...
  const renderHistoryPaymentModal = () => {
    if (!showHistoryPaymentModal || !selectedHistoryRental) return null;
        ...
        <Card className="w-full max-w-sm bg-slate-900 border-slate-800">
          ...
          <div className="space-y-4">
            <p className="text-slate-300">
              Update payment status for rental on
              <span className="text-white font-bold ml-1">
                {new Date(selectedHistoryRental.startTime).toLocaleDateString()}
              </span>?
            </p>
            ...
            <div className="bg-slate-800 p-3 rounded-lg border border-slate-700">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-white font-bold">{selectedHistoryRental.customerName}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400 text-sm">Total</span>
                <span className="text-emerald-400 font-mono font-bold">{APP_SETTINGS.currency}{selectedHistoryRental.totalPrice.toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <Button
                variant="secondary"
                onClick={() => setShowHistoryPaymentModal(false)}
                className="flex-1"
              >
                Cancel
              </Button>
              <Button
                onClick={async () => {
                  await togglePaymentStatus(selectedHistoryRental.id, selectedHistoryRental.isPaid);
                  setShowHistoryPaymentModal(false);
                }}
                className={`flex-1 ${selectedHistoryRental.isPaid ? 'bg-rose-600 hover:bg-rose-500' : 'bg-emerald-600 hover:bg-emerald-500'}`}
              >
                Mark as {selectedHistoryRental.isPaid ? 'UNPAID' : 'PAID'}
              </Button>
            </div>
          </div>
        </Card>
                    ...
                    {catProducts.map(product => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div className={`p-1.5 rounded-md ${cat === ProductCategory.DRINK ? 'bg-blue-500/20 text-blue-400' :
